...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(plugin.video.antares)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.antares').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(plugin.video.antares)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://plugin.video.antares)')
...
def metaFile():
    if condVisibility('System.HasAddon(plugin.video.antares)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.antares').getAddonInfo('path'), 'resources', 'data', 'meta.db')
